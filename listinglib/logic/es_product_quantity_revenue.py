...
from listinglib.config import DatabaseConfig

__author__ = 'TungLQ'
_logger = logging.getLogger(__name__)


class EsProductQuantityRevenue:
    @classmethod
    def collect_quantity_and_revenue_by_pv_sku(self):
        # join pv_sku voi tien va so luong hang ban ra
        pv_sku_df = self._collect_product_sku()
        quan_reve_df = self._collect_quantity_and_revenue()
        ...
        normalized_df['revenue'] = normalized_df['tien_ban_ra'].apply(
            self.normalize_func(tien_ban_ra_min, tien_ban_ra_max))
        normalized_df['quantity'] = normalized_df['sl_ban_ra'].apply(
            self.normalize_func(sl_ban_ra_min, sl_ban_ra_max))
    ...
    @classmethod
    def _collect_quantity_and_revenue(self):
        ...
        sql_query = self._build_aggregate_sale_quantity_and_revenue_query()
